 ...
 */
export function parseScanUrl(input: string): URL {
  if (typeof input !== "string") throw new UnsafeUrlError("URL must be a string");
  const trimmed = input.trim();
  if (!trimmed) throw new UnsafeUrlError("URL is empty");
  if (trimmed.length > MAX_URL_LENGTH) throw new UnsafeUrlError(`URL exceeds ${MAX_URL_LENGTH} characters`);
  ...
  if (!u.hostname) throw new UnsafeUrlError("URL has no host");
  if (u.hostname.length > MAX_HOST_LENGTH) throw new UnsafeUrlError("Host is too long");
  u.hash = "";
  return u;
}
...
function isBlockedV4(ip: string): boolean {
  ...
  if (a === 192 && b === 0) return true; // 192.0.0.0/24 + 192.0.2.0/24 reserved/TEST-NET
...
function isBlockedV6(ip: string): boolean {
  ...
  if (ip.includes("::ffff:") || ip.startsWith("64:ff9b:")) return true;
  return false;
}

export interface ResolvedSafeUrl {
  url: URL;
  /** The resolved IP we verified as public — callers should pin to this. */
  ip: string;
  family: number;
}
 ...
 */
export async function assertResolvableAndPublic(u: URL): Promise<ResolvedSafeUrl> {
  ...
  if (literal) {
    ...
    const testLoopback =
      host === "127.0.0.1" && process.env.ASO_SCANNER_TEST_ALLOW_LOOPBACK === "1";
  ...
  if (!addrs.length) throw new UnsafeUrlError(`No DNS records for ${host}`);
